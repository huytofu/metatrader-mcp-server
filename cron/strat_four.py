...
def setup_logging():
    ...
    # Main strategy log with rotation - separate file for strat_four
    log_file = os.path.join(log_dir, 'range_straddle_strategy_four.log')
...
STRATEGY_CONFIG = {
    'symbol': 'GBPUSD',  # Best performing pair from historical tests
...
class RangeStraddleStrategy:
    ...
    def analyze_range_opportunity(self, candles_df):
        ...
        try:
            ...
            # Convert range to pips (for GBPUSD, 1 pip = 0.0001)
...
def setup_windows_task():
    ...
    # Task name - make it unique for strat_four
    task_name = f"RangeStraddleStrategy_{STRATEGY_CONFIG['symbol']}_{STRATEGY_CONFIG['timeframe']}_Four"
...
def setup_unix_cron():
    ...
    try:
        ...
        cron.remove_all(comment='range_straddle_strategy_four')
        ...
        job = cron.new(command=command, comment='range_straddle_strategy_four')
        ...
        for job in cron:
            if job.comment == 'range_straddle_strategy_four':
                logger.info(f"   {job}")
